...
manana = (datetime.today() + timedelta(days=1)).date()
hoyutc = datetime.now(timezone.utc).date()
...
def verificar_registros_manana():
    """Verifica si hay registros en DUC_VALMD para mañana."""
    conn = pyodbc.connect(conn_str, autocommit=True)
    cursor = conn.cursor()

    query = f"""
    SELECT COUNT(*) 
    FROM [METDB].[MET_DATOSUPCOMB] 
    WHERE DUC_FECHA = '{manana}'
    AND DUC_VALMD IS NOT NULL;
    """
    ...
    if resultado > 0:
        print(f"Se encontraron {resultado} registros en DUC_VALMD para mañana ({manana}). El proceso NO se ejecutara.")
        return False 
    else:
        print(f"No se encontraron registros en DUC_VALMD para mañana ({manana}). Se procedera con la ejecucion.")
        return True

def procesar_xmls_y_insertar():
    """Procesa archivos XML y los inserta en la base de datos."""
    print("")
    ...
    data_total = []
    procesar_mas_archivos = True  # Bandera para detener el procesamiento si se cumple la condición
    # Procesar cada archivo XML
    for archivo in archivos_xml:
        if not (archivo.startswith("MD_") and archivo.endswith("_RAW.xml")):
            continue
        if not procesar_mas_archivos:
            break  # Si ya encontramos una fecha > hoy, no seguimos procesando más archivos
        ...
        data = []
        archivo_valido = False  # Bandera para determinar si este archivo tiene fechas >= hoy
            ...
            if up is not None:
                ...
                if periodo_elem is not None:
                    ...
                    if intervalo_tiempo_elem is not None:
                        fecha_fin_completa = intervalo_tiempo_elem.get('v').split('/')[1]  # Tomar la fecha final completa con hora
                        fecha_fin_dt = datetime.strptime(fecha_fin_completa[:10], "%Y-%m-%d").date()

                        # Obtener la fecha actual en UTC
                        hoyutc = datetime.now(timezone.utc).date()


                        if fecha_fin_dt > hoyutc:
                            
                            archivo_valido = True  # Marcar este archivo como válido para ser procesado
                            hora = 1
                            for i, intervalo in enumerate(periodo_elem.findall(f'{namespace}Intervalo'), start=1):
                                ...
                                if i % 4 == 0:
                                    hora += 1
        if archivo_valido and data:
            print("\tDatos encontrados para el MD D+1 en el fichero: {}".format(archivo))
            # Imprimir valores para depuración
            print(f"\t[DEBUG] Archivo: {archivo}")
            print(f"\t[DEBUG] Fecha extraída del XML: {fecha_fin_dt}")
            print(f"\t[DEBUG] Fecha actual (hoy en UTC): {hoyutc}")
            df_temp = pd.DataFrame(data)
            data_total.append(df_temp)
            procesar_mas_archivos = False
            # Renombrar el archivo de _RAW.xml a _PROCESSED.xml
            archivo_procesado = archivo.replace('_RAW', '_PROCESSED')
            ruta_actual = os.path.join(ruta_carpeta, archivo)
            ruta_nueva = os.path.join(ruta_carpeta, archivo_procesado)
...
def main():
    """Función principal que ejecuta el proceso solo si no hay registros para mañana."""
    if verificar_registros_manana():
        procesar_xmls_y_insertar()
