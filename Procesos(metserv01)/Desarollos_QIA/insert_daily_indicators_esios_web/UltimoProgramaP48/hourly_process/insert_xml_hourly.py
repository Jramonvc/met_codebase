...
for archivo in archivos_xml:
    if not archivo.endswith('_RAW.xml') or archivo.startswith('MD_'):
        continue
...
try:
    archivos_xml = [f for f in os.listdir(ruta_carpeta) if f.endswith('_RAW.xml')]
    for archivo in archivos_xml:
        if not archivo.startswith('MD_'):
            archivo_path = os.path.join(ruta_carpeta, archivo)
            
            # Crear el nuevo nombre reemplazando _RAW con _PROCESSED
            nuevo_nombre = archivo.replace('_RAW', '_PROCESSED')
            nuevo_path = os.path.join(ruta_carpeta, nuevo_nombre)
            
            # Renombrar el archivo
            os.rename(archivo_path, nuevo_path)
            print(f"Archivo renombrado: {archivo} -> {nuevo_nombre}")
except Exception as e:
    print(f"Error al renombrar archivos XML: {e}")
