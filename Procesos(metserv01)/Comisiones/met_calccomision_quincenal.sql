/****** Object:  StoredProcedure [METDB].[MET_CALCCOMISION_QUINCENAL]    Script Date: 25/06/2025 12:09:17 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [METDB].[MET_CALCCOMISION_QUINCENAL] AS
DECLARE
...
BEGIN
   Declare Comisiones_Con Cursor For
    Select FPW_CNT_CODIGO, FPW_CLI_NIF, FPW_CUP_CODIGO, CAG_COMISION, CAG_AGE_CODIGO, FPW_NUMFACT, FPW_TOTCONS, FPW_TOTCONS*CAG_COMISION
	 ...
	 Where CAG_TIPCOMI = 'C'
	   And CAG_AGE_CODIGO  IN ('118','165','161','231')
  ...
  Declare Comisiones_C_T Cursor For
    Select cnt_codigo, CNT_CLI_NIF, CNT_CUP_CODIGO,CNT_POTP1,CPW_COMISION, CNT_AGE_CODIGO
     ...
     Where CNT_FECALT Between convert(date, '2024-05-01') and  getdate()
       And CNT_AGE_CODIGO IN ('118','165','231')
 ...
 DEALLOCATE Comisiones_C
   ...
   Declare Renovaciones Cursor For
     Select cnt_codigo, CNT_CLI_NIF, CNT_CUP_CODIGO,CNT_POTP1,
	        CASE CNT_AGE_CODIGO 
			  WHEN '165' Then CPW_RENOVA
			  ELSE
			   CASE sign(datediff(day, convert(date, '2023-07-01'), CNT_FECALT )) 
			    when 1 then CPW_RENOVA 
				else 60 
			  END
			END
			Renova, CNT_AGE_CODIGO
       From metdb.MET_CONTRATOS, metdb.MET_GRTARIF, METDB.MET_COMIAGENTPW
      Where  (CNT_CNS_CODIGO = 'C' 
        ...
        And CNT_POTP1 Between CPW_MINPOT AND CPW_MAXPOT 
		And CNT_FECALT Between CPW_FECINI And IsNull(CPW_FECFIN, CNT_FECALT)
		And ((day(getdate())= 15 and day(CNT_FECALT) between 1 and 15 
		         and format(CNT_FECALT, 'MM') in (FORMAT(dateadd(YEAR, -1, (DATEADD(MONTH,-1,GETDATE()))),'MM'), FORMAT(dateadd(YEAR, -1, (GETDATE())),'MM')))
		        or (day(getdate()) = 1 and day(CNT_FECALT) between 1 and 31 and MONTH(CNT_FECALT) = MONTH(DATEADD(MONTH,-1,GETDATE())) and YEAR(CNT_FECALT) <= YEAR(DATEADD(MONTH,-1,GETDATE())) - 1))
 ...
 Declare Decomisiones Cursor For
        Select cnt_codigo, CNT_CLI_NIF, CNT_CUP_CODIGO, CNT_POTP1, CCM_COMISION, CNT_AGE_CODIGO,
	       CASE WHEN DATEDIFF(DAY,CNT_FECALT, ISNULL(CNT_FECBAJ,CNT_FECVEN)) = 0 Then CCM_COMISION * -1
				ELSE
      ...
      From metdb.MET_CONTRATOS, metdb.MET_GRTARIF, METDB.MET_CALCCOMISION
     Where isnull(CNT_FECBAJ,CNT_FECVEN) Between DATEFROMPARTS(YEAR(DATEADD(MONTH, -1, getdate())),MONTH(DATEADD(MONTH, -1, getdate())),1) And GETDATE()
       And CNT_AGE_CODIGO IN ('118','165','231')
       And CNT_POTP1 is not null
  ...
  While @@Fetch_Status = 0 
   ...
   DEALLOCATE Decomisiones  
    ...
    SELECT CAG_AGE_CODIGO, CAG_CNT_CODIGO, CNT_CLI_NIF, CNT_CUP_CODIGO, CAG_COMISION, isnull(CAG_POTENCIA,0), CAG_CONSUMO, (CNT_POTP1 + CNT_POTP2 + ISNULL(CNT_POTP3,0) + ISNULL(CNT_POTP4,0) + ISNULL(CNT_POTP5,0) + ISNULL(CNT_POTP6,0)) Potencia
	  FROM METDB.MET_CONTRACTCOMIAG, METDB.MET_CONTRATOS
	  WHERE CAG_TIPCOMI = 'O'
	   AND CAG_AGE_CODIGO IN ('118','165','231')
    ...
    Begin
      ...
      Insert into METDB.MET_CALCCOMISION (CCM_ID,         CCM_FECPAGO,              CCM_CNT_CODIGO, CCM_CLI_NIF,  CCM_CUP_CODIGO, 
								  ...
								  Values (@N_ID,          convert(date, getdate()), @N_Codcontrato, @V_Cliente,   @V_Cup,
										  ...
										  @N_Consumo,     (@N_Comision * @N_Consumo) + (@N_MargPot * @N_Potencia/1000))   
     FETCH NEXT From Comisiones_O Into @V_Agente, @N_Codcontrato, @V_Cliente, @V_Cup, @N_Comision,  @N_MargPot, @N_Consumo, @N_Potencia; 
	End
  Close Comisiones_O
  DEALLOCATE Comisiones_O
  Declare Decomision_O Cursor For 
      SELECT CAG_AGE_CODIGO, CAG_CNT_CODIGO, CNT_CLI_NIF, CNT_CUP_CODIGO, CAG_COMISION, CAG_POTENCIA, CAG_CONSUMO, (CNT_POTP1 + CNT_POTP2 + ISNULL(CNT_POTP3,0) + ISNULL(CNT_POTP4,0) + ISNULL(CNT_POTP5,0) + ISNULL(CNT_POTP6,0)) Potencia, CNT_FECALT  
	    FROM METDB.MET_CONTRACTCOMIAG, METDB.MET_CONTRATOS C1
       WHERE CAG_TIPCOMI = 'O'
         AND CAG_AGE_CODIGO IN ('118','165','231')
	     AND CAG_CNT_CODIGO = CNT_CODIGO
	     AND CNT_CNS_CODIGO = 'B'
	     AND ISNULL(CNT_FECBAJ,CNT_FECVEN)  between DATEFROMPARTS(YEAR(DATEADD(MONTH, -2, getdate())),MONTH(DATEADD(MONTH, -2, getdate())),1) And EOMONTH(DATEADD(MONTH, -2, getdate()))
